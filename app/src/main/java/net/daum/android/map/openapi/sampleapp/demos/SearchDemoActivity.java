...
public class SearchDemoActivity extends FragmentActivity implements MapView.MapViewEventListener, MapView.POIItemEventListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mMapView.setPOIItemEventListener(this); //marker
        mMapView.setCalloutBalloonAdapter(new CustomCalloutBalloonAdapter()); //balloon
        ...
        mButtonSearch.setOnClickListener(new OnClickListener() { // 검색버튼 클릭 이벤트 리스너
			...
			public void onClick(View v) {
		        ...
		        GeoCoordinate geoCoordinate = mMapView.getMapCenterPoint().getMapPointGeoCoord(); //map중심점 좌표값
    ...
    public void onMapViewInitialized(MapView mapView) {
        ...
        mMapView.setMapCenterPointAndZoomLevel(MapPoint.mapPointWithGeoCoord(37.1935892,127.02086), 2, true);
        
        Searcher searcher = new Searcher();
        String query = mEditTextQuery.getText().toString();
        double latitude = 37.1935892; //위도
        double longitude = 127.02086; //경도
        int radius = 10000; // 중심 좌표부터의 반경거리. 특정 지역을 중심으로 검색하려고 할 경우 사용. meter 단위 (0 ~ 10000)
        int page = 1;
        String apikey = MapApiConst.DAUM_MAPS_ANDROID_APP_API_KEY;
	...
	//marker위의 balloon을 클릭할 때 태그로 나오는 정보
	@Override
